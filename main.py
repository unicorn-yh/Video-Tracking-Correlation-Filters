...
def test_tracker(sequence_path, params, save, ground_truth_path):
    ...
    while(frame_reader.good()):
        ...
        if frame_counter == 0:
            ...
            if save:
                video_writer = cv.VideoWriter('result/'+sequence_path.replace('.mp4','').replace('video/','')+'_output.mp4', cv.VideoWriter_fourcc('M','J','P','G'), 30, (curr_frame.shape[1], curr_frame.shape[0]))
...
def main():
    sequence_path = 'video/basketball.mp4'
